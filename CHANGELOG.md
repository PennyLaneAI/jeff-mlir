...
This release fixes the linearity of `WhileOp` (see https://github.com/unitaryfoundation/jeff/issues/4 for more information). As the new implementation covers the functionalit of a do-while operation, `DoWhileOp` has been removed.

Furthermore, this release marks `SwitchOp`, `ForOp`, and `WhileOp` as `IsolatedFromAbove`, ensuring that their regions are checked to be isolated from above.
